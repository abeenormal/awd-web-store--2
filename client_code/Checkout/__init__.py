...
class Checkout(CheckoutTemplate):
  ...
  def buy_click(self, **event_args):
    ...
    if anvil.users["purchased_courses"] and self.course["id_name"]in anvil.user["purchased_courses"]:
     alert("You already own this course!")
      
  token, info=stripe.checkout.get_token(amount= course["price"]*100, currency="USD",title=self.course["name"], description=self.course["description"])
  try:
